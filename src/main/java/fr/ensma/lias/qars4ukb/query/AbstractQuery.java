...
public abstract class AbstractQuery implements Query {
	...
	if (!rdfQuery.equals("")) {
		...
		triplePatterns.add(new TriplePattern(
			rdfQuery.substring(indexOfLeftEmbrace + 2, indexOfDot),
	    ...
	    triplePatterns
		    .add(new TriplePattern(
			    rdfQuery.substring(indexOfLeftEmbrace + 2,
				    rdfQuery.length() - 2),
			    indiceOfTriplePattern));
	}
    }
	...
	List<TriplePattern> listTP = new ArrayList<TriplePattern>(
		this.triplePatterns);
	listTP.addAll(queryToConcat.getTriplePatterns());
	final Query createQuery = factory.createQuery(computeRDFQuery(listTP),
		newInitialQuery);
	return createQuery;
    }
	...
	if (rdfQuery.equals(""))
	    ...
	    rdfQuery = rdfQuery.substring(0, rdfQuery.length() - 1) + ". "
		    + tp.toString() + " }";
	}
    }
	...
	final int prime = 31;
	int result = 1;
	result = prime * result + ((triplePatterns == null) ? 0
		: new HashSet<TriplePattern>(triplePatterns).hashCode());
	return result;
    }
    ...
    protected void startAlgorithm(Session session,
	    ComputeMFSAndXSSAlgorithm algo) throws Exception {
	switch (algo) {
	case LBA:
	    runLBA(session);
	    break;
	...
	for (int i = 0; i < triplePatterns.size(); i++) {
		...
		res += "t"
			+ (initialQuery.getTriplePatterns().indexOf(temp) + 1);
	}
	return res;
    }
    ...
    private boolean isFailingForDFS(Map<Query, Boolean> executedQueries,
	...
	while (!listQuery.isEmpty()) {
		...
		if (((AbstractQuery) qTemp).isFailingForDFS(executedQueries,
			session)) {
		    // this is a potential MFS
		    // System.out.println("potential mfs");
		    boolean isMFS = true;
		    for (Query subquery : subqueries) {
			if (((AbstractQuery) subquery)
				.isFailingForDFS(executedQueries, session))
			    isMFS = false;
		    }
		    if (isMFS)
			allMFS.add(qTemp);
