...
public class ExperimentAlgoTest {
	...
	public void testJena() {
		...
		// cette ligne on la rajoute pour la version Graph et non pas pour la
		// reification
	 ...
	 * JDBCQueryExtFactory(); testAlgo(Algorithm.BOTTOMUP, PLAT_JENA, listOfAlpha);
	 * }
	 * 
	 * @Test public void testHSQLDBTopDown() throws Exception { factory = new
	 * JDBCQueryExtFactory(); testAlgo(Algorithm.TOPDOWN, PLAT_JENA, listOfAlpha); }
	 * 
	 * @Test public void testHSQLDBHybrid() throws Exception { factory = new
	 * JDBCQueryExtFactory(); testAlgo(Algorithm.HYBRID, PLAT_JENA, listOfAlpha); }
	 */
	/************************************
	 ...
	 * 
	 * @param q        the query
	 * @param typeAlgo the used algorithm
	 */
	 ...
	 * List<QueryExplain> newTestResultPairList = null;
	 * 
	 * newTestResultPairList = this.newTestResultPairList("/" + FILE_QUERIES); for
	 * (int i = 0; i < newTestResultPairList.size(); i++) { QueryExplain qExplain =
	 * newTestResultPairList.get(i); Query q = qExplain.getQuery(); // String
	 * description = qExplain.getDescription(); // System.out.println("Query (" +
	 * description + "): "); // NLBA factory = new JDBCQueryOptFactory(); q =
	 * factory.createQuery(q.toString()); algo = new AlgoNLBA(); AlgoResult
	 * algoResultNLBA = algo.computesAlphaMFSsAndXSSs(q, listOfAlpha); // HYBRID
	 * factory = new JDBCQueryExtFactory(); q = factory.createQuery(q.toString());
	 * algo = new AlgoHybrid(); AlgoResult algoResultHybrid =
